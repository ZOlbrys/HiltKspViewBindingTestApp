# HiltKspViewBindingTestApp

Building the app on the `replace-kapt-with-ksp` branch will result in build errors.
...
e: [ksp] InjectProcessingStep was unable to process 'testClass' because 'error.NonExistentClass' could not be resolved.

Dependency trace:
    => element (CLASS): com.example.hiltkspviewbindingtestapp.MainActivity
    => type (DECLARED superclass): com.example.hiltkspviewbindingtestapp.BaseActivity<error.NonExistentClass>
